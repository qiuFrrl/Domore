...
public:
    ...
    void stopHome();
    void updateHome(uint32_t nowMs, const TimeSnapshot &time, const WeatherData &weather, const BatteryStatus &battery);
...
private:
    AnimationId nextHomeAnimation(const TimeSnapshot &time, const WeatherData &weather);
    ...
    uint32_t _lastLowBatteryAnimationAt = 0;
    uint8_t _homeStep = 0;
