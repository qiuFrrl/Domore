...
- Battery monitor nanti -> GPIO3 lewat voltage divider, bukan langsung dari LiPo.

## Struktur Folder

- `src/main.cpp`: entry point Arduino. Hanya start Serial, panggil `RobodeskApp::begin()`, lalu `RobodeskApp::update()`.
- `include/config/AppConfig.h`: semua konfigurasi pin, WiFi, interval tombol, delay random animasi, NTP, lokasi fallback, weather refresh, dan threshold baterai.
...
- `include/system` dan `src/system`: WiFi, NTP time, weather API, lokasi, dan baterai.
...
- `src/app/RobodeskApp.cpp`: mengatur boot sequence `Intro.h` -> tahan frame akhir 2 detik -> `Excited` sekali, cek birthday sekali setelah boot intro selesai, update manager, pindah screen, input tombol, dan render display.
...
- `src/animation/AnimationSets.cpp`: tempat edit animasi default, animasi tap 1/3/5/7/9, hold, AFK, wake, low battery, birthday final, rule jam, dan rule cuaca.
- `include/animation/DomoreAnimationManager.h`: manager animasi Domore.
- `src/animation/DomoreAnimationManager.cpp`: memilih animasi default secara random dari pool, delay random 1-10 detik, menjalankan `Blank` sebagai animasi penunggu setelah setiap one-shot selesai, menambah animasi dari rule jam/cuaca, menjalankan low battery, dan mode AFK 2 menit yang loop sampai tombol disentuh.
...
- `src/display/BirthdayScene.cpp`: gambar kue, kembang api, bunga, teks birthday. Scene ini dipanggil sekali per boot setelah intro selesai jika tanggal hari ini adalah 7 Oktober.
...
- `src/menu/MenuManager.cpp`: menu sementara: `Domore`, `Time`, `Weather`.

## System

- `include/system/WifiTypes.h`: struct SSID dan password.
- `include/system/WifiManager.h`: state machine WiFi non-blocking.
- `src/system/WifiManager.cpp`: koneksi WiFi fallback ke `bitha` dan `Lich 5G`, retry tanpa blocking loop.
...
- `include/system/BatteryManager.h`: status baterai, voltage, persen, dan low flag.
- `src/system/BatteryManager.cpp`: pembacaan ADC baterai lewat voltage divider dan estimasi persen.
...
- Boot: OLED tidak menampilkan teks `ROBODESK`/`Starting...`; screen `Boot` langsung menggambar animasi `Intro.h` lewat `AnimationPlayer`, tahan frame terakhir 2 detik, lalu menjalankan `Excited` sekali sebelum masuk Domore.
- Domore normal: pilih random dari 5 animasi default, jalankan animasi sekali, lalu loop `Blank` selama menunggu delay random 1-10 detik selesai.
...
- AFK: jika tombol tidak disentuh 2 menit, jalankan satu animasi loop terus sampai tombol disentuh.
...
- Low battery: jika baterai low, panggil animasi khusus dan ulangi reminder dengan interval.
- 7 Oktober: setelah boot intro selesai, app cek tanggal sekali. Kalau tanggal hari ini 7 Oktober dan belum pernah dimainkan di boot ini, tampilkan birthday scene, lalu lanjut satu animasi final dari file animation. Jika robot dimatikan lalu dinyalakan lagi pada hari yang sama, birthday scene akan muncul lagi sekali setelah intro.

## Algoritma Membuat Robot dari 0

1. Buat project PlatformIO ESP32-C3 dengan Arduino framework.
2. Buat `include/config/AppConfig.h` untuk semua konstanta awal: pin OLED, pin TTP223, WiFi credential, interval tombol, interval weather, NTP, lokasi fallback, delay animasi random, durasi tahan frame akhir intro, baterai, dan tanggal birthday.
...
10. Buat `include/menu/MenuManager.h` dan `src/menu/MenuManager.cpp`; simpan daftar item menu dan action seperti `Domore`, `Time`, dan `Weather`.
11. Buat manager system satu per satu: `WifiManager`, `TimeManager`, `LocationManager`, `WeatherManager`, dan `BatteryManager`.
...
13. Buat `include/animation/AnimationSets.h` dan `src/animation/AnimationSets.cpp`; isi pool animasi default, mapping tap count, animasi hold, animasi AFK, low battery, birthday, rule jam, dan rule cuaca.
14. Buat `include/animation/DomoreAnimationManager.h` dan `src/animation/DomoreAnimationManager.cpp`; urus random idle, delay random, filler `Blank`, tap one-shot, hold one-shot, AFK loop, wake animation, low battery reminder, dan pemilihan animasi dari rule jam/cuaca.
...
16. Buat `include/app/RobodeskApp.h` dan `src/app/RobodeskApp.cpp`; ini orkestrator utama yang memanggil semua manager, mengatur routing screen, input, menu action, boot sequence `Intro.h` -> hold 2 detik -> `Excited`, cek birthday sekali setelah intro, Domore, weather, time, dan render.
17. Build dengan `pio run -j1`; perbaiki compile error dari header include, enum animasi, dan dependency library.
18. Flash ke ESP32-C3, buka Serial Monitor, validasi urutan: boot animation `Intro.h`, frame akhir intro tahan 2 detik, `Excited` sekali, cek birthday sekali jika tanggal 7 Oktober, WiFi connect, time sync, weather update, menu tampil, tap animation, Blank filler, hold, menu hold, dan AFK.
